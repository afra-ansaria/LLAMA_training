...
def main(**kwargs):
    ...
    if train_config.quantization:
        if isinstance(train_config.quantization, bool):
            warn(
                "Quantization (--quantization) is a boolean, please specify "
                "quantization as '4bit' or '8bit'. Defaulting to '8bit'.",
                FutureWarning,
            )
            train_config.quantization = "8bit"

        if train_config.quantization == "8bit" and train_config.enable_fsdp:
            raise ValueError("8bit quantization is not supported with FSDP; use 4bit instead.")
    ...
    if len(tokenizer) > model.get_input_embeddings().weight.shape[0]:
        print("Resizing embedding matrix to match tokenizer vocab size.")
    ...
    dataset_processer = processor if "processor" in locals() else tokenizer
    ...
    if train_config.batching_strategy == "packing":
        if is_vision:
            raise ValueError("Packing not supported for vision datasets")
    ...
    print(f"Num of training batches = {len(train_dataloader)}")

    eval_dataloader = None
    if train_config.run_validation:
        if train_config.batching_strategy == "packing" and is_vision:
            raise ValueError("Packing not supported for vision datasets")
        ...
        print(f"Num of validation batches = {len(eval_dataloader)}")
